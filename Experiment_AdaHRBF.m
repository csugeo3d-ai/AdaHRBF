...
%Approximate minima in calculation(useless in current version) 
k=1e-4;
%Set basefunction type(useless in current version) 
basefunctype=1;

% Get initial gradient magnitude from attitude points
...
dataMax=max([attributes_north(:,[1,2,3]);attitude_north(:,[1,2,3]);attributes_south(:,[1,2,3]);attitude_south(:,[1,2,3])]);
dataMin=min([attributes_north(:,[1,2,3]);attitude_north(:,[1,2,3]);attributes_south(:,[1,2,3]);attitude_south(:,[1,2,3])]);
[meshgrid_NX,meshgrid_NY,meshgrid_NZ] = meshgrid(dataMin(1,1):100:dataMax(1,1),dataMin(1,2):100:dataMax(1,2),0:25:dataMax(1,3));
[meshgrid_SX,meshgrid_SY,meshgrid_SZ] = meshgrid(dataMin(1,1):100:dataMax(1,1),dataMin(1,2):100:dataMax(1,2),0:25:dataMax(1,3));



%%
%HRBF: Interpolate a 3D scalar field function in northern sub-domain
...
%HRBF: Interpolate a 3D scalar field function in southern sub-domain 
[alphs,betas,charlies]=GetParameters(1e-4,1,attributes_south,gradient_south);
valueGrid_south=GetValueGrid(k,basefunctype,meshgrid_SX,meshgrid_SY,meshgrid_SZ,attributes_south,alphs,charlies,gradient_south,betas);
%%
%adaHRBF: Interpolate a 3D scalar field in northern sub-domain
...
%adaHRBF: Interpolate a 3D scalar field in southern sub-domain
...
opt_valueGrid_south=GetValueGrid(k,basefunctype,meshgrid_SX,meshgrid_SY,meshgrid_SZ,attributes_south,opt_alph_s,opt_charlie_s,opt_gradient_south,opt_bravo_s);
%%
%RBF: Interpolate a 3D scalar field in fault area
[alph,~,charlie]=GetParameters(1e-4,1,attributes_fault);
valueGrid_fault=GetValueGrid(k,basefunctype,meshgrid_NX,meshgrid_NY,meshgrid_NZ,attributes_fault,alph,charlie);
%%
...
%Remove grids outside the study area
...
index=find(meshgrid_SY<=-1.21*meshgrid_SX+3379598.04 | meshgrid_SY>=-1.21*meshgrid_SX+3389045.69 | meshgrid_SY>=0.83*meshgrid_SX+2028031.65 | meshgrid_SY<=0.83*meshgrid_SX+2011701.6);%获得位于矩形建模区域外格网坐标
valueGrid_south(index)=nan;
opt_valueGrid_south(index)=nan;

%Remove grids outside the study area 
bar = waitbar(0,'Removing grids outside the study area...'); 
for i=1:size(meshgrid_NZ,1) 
    ...
    waitbar(i/(size(meshgrid_NZ,1)),bar,'Removing grids outside the study area...')    
end
close(bar)
bar = waitbar(0,'Removing grids outside the study area...'); 
for i=1:size(meshgrid_SZ,1)
    ...
    waitbar(i/(size(meshgrid_SZ,1)),bar,'Removing grids outside the study area...')    
end
close(bar)
%%
...
%Dsiplay stratigraphic interfaces in the northern subdomain
fig2=figure('Name','HRBFisosurfaces');         
figure(fig2);
%With unoptimized gradient magnitude
subplot(1,2,1);
...
fig3=figure('Name','HRBFscatters');
figure(fig3);
%Dsiplay scalar field in the southern subdomain
figure(fig3);
%With unoptimized gradient magnitude
subplot(1,2,1);
...
%Dsiplay stratigraphic interfaces in the southern subdomain
figure(fig2);
%With unoptimized gradient magnitude
subplot(1,2,1);
for i=8:12
